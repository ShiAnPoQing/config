...
local function float_buffer_manage()
...
local function split_buffer_manage(position)
  if not is_split(position) then
    return
  end
  local current_buf = vim.api.nvim_get_current_buf()
  local current_win = vim.api.nvim_get_current_win()
  ...
  local win = vim.api.nvim_open_win(Buffer.buf, true, win_config)
  Buffer:update(win, current_buf)
  ...
  vim.api.nvim_create_autocmd("BufHidden", {
    callback = function(ev)
      del_autocmd({ BufWinEnter, WinEnter })
      return true
    end,
    buffer = Buffer.buf,
  })
...
function M._buffer_manage(opt)
  local position = opt.position or config.position
  if position == "float" then
    float_buffer_manage()
    return
  end
  split_buffer_manage(position)
end

function M.buffer_manage() end
