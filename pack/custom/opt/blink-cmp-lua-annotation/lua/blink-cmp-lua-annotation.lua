...
-- `opts` 表来自 `sources.providers.your_provider.opts`
-- 您还可以接受第二个参数“config”，以获取完整的
-- `sources.providers.your_provider` 表
...
function source:get_completions(ctx, callback)
  ...
  for i = 1, 10 do
    ...
    local item = {
      ...
      -- 或者让blink.cmp 为您猜测要替换的范围。仅使用此
      -- 当插入*仅*字母数字字符时。任何符号都会
      -- 在blink.cmp中触发复杂的猜测逻辑，可能不会给出
      -- 您期望的结果
      -- 请注意，当省略 `insertText` 和 `textEdit` 时，blink.cmp 将使用 `label`
      insertText = "foo",
...
--（可选）在接受项目或显示文档之前，blink.cmp 将调用此函数
-- 因此您可以避免仅在实际需要时才计算昂贵的字段（即文档）
-- 请注意，只有某些字段可能会延迟解析。您可以查看 LSP 功能的完整列表：
-- `textDocument.completion.completionItem.resolveSupport`
-- 在撰写本文时：“文档”、“详细信息”、“additionalTextEdits”、“命令”、“数据”
...
--（可选）在应用项目的textEdit/insertText后立即调用
-- 仅当您想要自定义项目的接受方式时才有用，
-- 超出了 `textEdit` 和 `additionalTextEdits` 的范围
