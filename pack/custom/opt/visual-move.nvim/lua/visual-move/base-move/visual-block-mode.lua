...
local Select = require("visual-move.utils.visual-block-mode")
local Line = {}
...
function Line:set_lines(start_row, start_col, end_row, end_col, cursor_pos)
  if self.stop == true then
    set_visual_mark(start_row, start_col, end_row, end_col)
  ...
  set_visual_mark(start_row, start_col + start_col_offset, end_row, end_col + end_col_offset)
  select(cursor_pos, start_col)
end
