...
local function on_del_line(bufnr, del_count, firstline, lastline, new_lastline)
    ...
    for i = break_index, #Extmark.extmarks do
      ...
      if firstline <= mark.row and lastline >= mark._next.row then
        ...
        table.insert(a, function()
          BufAttch:stop()
          print(mark.name, mark.row)
          vim.api.nvim_buf_del_extmark(Float.buf, Extmark.ns_id, mark.id)
          -- vim.api.nvim_buf_set_lines(Float.buf, mark.row, mark.row, false, { "不可删除" })
          vim.api.nvim_buf_set_extmark(Float.buf, Extmark.ns_id, mark.row, 0, {
            virt_lines = {
              {
                { mark.name .. " 寄存器：", "CursorLineNr" }
              },
  ...
  vim.schedule(function()
    ...
    Extmark:update_extmarks(bufnr, break_index)
    if #a > 0 then
      vim.api.nvim_buf_set_lines(bufnr, start_mark.row, start_mark.row, false, l)
      for _, callback in ipairs(a) do
        callback()
      end
    end
    update_registers()
